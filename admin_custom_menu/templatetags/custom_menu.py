...
class Menu(object):
    ...
    def init_config(self):
        exclude = get_config('MENU_EXCLUDE') or []
        self.conf_exclude = exclude + ['admin', 'ra.activity']
        self.conf_open_first_child = get_config('MENU_OPEN_FIRST_CHILD')
        self.conf_icons = get_config('MENU_ICONS')
        self.conf_labels = get_config('MENU_LABELS')
    ...
    def activate_menu(self, menu):
        ...
        if not self.app_activated and not self.request.path.startswith('/reports/'):
            for app in menu:
